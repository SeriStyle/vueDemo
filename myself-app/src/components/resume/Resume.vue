<template>
  <div class="content">
    <div>
      <h1 class="title">{{getUserName}}</h1>
      <p class="slogan">{{text.slogan}}</p>
      <ul class="navi">
        <li>
          <router-link to="/wechat" exact>WeChat</router-link>
        </li>
        <li>
          <router-link to="/aboutlist/showArtical" exact>AboutList</router-link>
        </li>
        <li>
          <a href="https://vuex.vuejs.org/zh/" target="_blank">Vue.js</a>
        </li>
        <li>
          <a href="mailto:styhss@qq.com">Email</a>
        </li>
      </ul>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      loading: true,
      img: "",
      navidata: [{ href: "/wechat", text: "WeChat" }],
      text: {
        title: "Hei ! Guys",
...
.background::after {
  background: linear-gradient(
    ...
    0 / 1px
      url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2 2"><g><rect x="0" y="0" width="1" height="1"/><rect x="1" y="1" width="1" height="1"/></g></svg>');
}
.navi {
